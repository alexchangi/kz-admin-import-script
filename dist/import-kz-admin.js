...
const cloneOrUpdateRepo = () => {
    ...
    if (fs.existsSync(path.join(DESTINATION_PATH, '.git'))) {
        ...
        (0, child_process_1.execSync)(`git -C ${DESTINATION_PATH} pull`, { stdio: 'inherit' });
    }
    else {
        console.log('Cloning the React app repository...');
        (0, child_process_1.execSync)(`git clone ${REACT_APP_REPO_URL} ${DESTINATION_PATH}`, {
            stdio: 'inherit',
        });
    }
    console.log('Removing .git directory from the copied React app...');
    (0, child_process_1.execSync)(`rm -rf ${DESTINATION_PATH}/.git`, { stdio: 'inherit' });
    console.log("React app has been successfully imported into the 'apps/admin' directory.");
};
cloneOrUpdateRepo();
